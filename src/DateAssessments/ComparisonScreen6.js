...
const ComparisonScreen6 = (props) => {
  const [answers, setAnswers] = useState({});
  const styles = dateAssessmentStyles();
  const fieldName = 'AlcoholDrugs';
  ...
  const onFormSubmit = () => {
    ...
    getDocument('users', props.user.id).then((result) => {
      ...
      //navigate to next
      props.navigation.replace('ComparisonComplete');
    });
  };
...
ComparisonScreen6.navigationOptions = {
...
export default connect(mapStateToProps)(ComparisonScreen6);
