...
const ComparisonScreen5 = (props) => {
  const [answers, setAnswers] = useState({});
  const styles = dateAssessmentStyles();
  const fieldName = 'SexualBoundaries';
    ...
    sections: [
      {
        ...
        fields: [
          {
            displayName: 'Respect for sexual boundaries',
            type: 'doubleSlider',
  ...
  const onFormSubmit = () => {
    ...
    getDocument('users', props.user.id).then((result) => {
      ...
      //navigate to next
      props.navigation.replace('Comparison6');
    });
  };
...
ComparisonScreen5.navigationOptions = {
...
export default connect(mapStateToProps)(ComparisonScreen5);
