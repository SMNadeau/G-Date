...
const ComparisonScreen3 = (props) => {
  const [answers, setAnswers] = useState({});
  const styles = dateAssessmentStyles();
  const fieldName = 'RelationalStyle';
  ...
  const onFormSubmit = () => {
    ...
    getDocument('users', props.user.id).then((result) => {
      ...
      //navigate to next
      props.navigation.replace('Comparison4');
    });
  };
...
ComparisonScreen3.navigationOptions = {
...
export default connect(mapStateToProps)(ComparisonScreen3);
