...
const ComparisonScreen4 = (props) => {
  const [answers, setAnswers] = useState({});
  const styles = dateAssessmentStyles();
  const fieldName = 'FeelingsAboutSexualIdentity';
    ...
    sections: [
      {
        ...
        fields: [
          {
            displayName: 'Feelings about their sexual identity',
            type: 'doubleSlider',
            options: ['1', '2', '3', '4', '5', '6', '7'],
            displayOptions: [
              'Many issues with identity',
              ' ',
              ' ',
              'Average',
              ' ',
              ' ',
              'Very comfortable with identity',
            ],
            editable: true,
            key: fieldName,
          },
  ...
  const onFormSubmit = () => {
    ...
    getDocument('users', props.user.id).then((result) => {
      ...
      //navigate to next
      props.navigation.replace('Comparison5');
    });
  };
...
ComparisonScreen4.navigationOptions = {
...
export default connect(mapStateToProps)(ComparisonScreen4);
