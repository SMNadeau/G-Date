...
class ChatBot extends Component {
  ...
  onButtonPress() {
    if (!this.props.timerFinished) {
      //if timer is not finished, disable the form while we wait.
      this.setState({
        editable: false,
      });
    } else {
      //timer has finished, submit the message
      const {
        renderedSteps,
        previousSteps,
        inputValue,
      ...
      const isInvalid =
        currentStep.validator && this.checkInvalidInput(inputValue);
  ...
  checkInvalidInput(input) {
    ...
    const result = currentStep.validator(input);
    const value = input;
